...
os.chdir('..')
print(os.getcwd())
splice_table = './data/canonical_dataset.txt'
...
assert sys_argv[1] in ['train', 'test', 'all']
assert sys_argv[2] in ['0', '1', 'all']
...
with open(splice_table, 'r') as fpr1:
    for line1 in fpr1:
        ...
        if (sys_argv[2] != data1[1]) and (sys_argv[2] != 'all'):
            continue
